...
\begin{frame}{Q\&A: Hellinger Distance Convexity}
    ...
    \begin{block}{Answer}
        \begin{itemize}
            ...
            \item $\dH(P, Q)$ is generally \textbf{not convex} but \textbf{concave} in each argument
            \item What the proof actually uses: $P_X^{(k)}$ is convex conbination of $P_{X|W}(\cdot|w)$
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Q\&A: Hellinger Distance Convexity}
    \begin{block}{Answer}
        \begin{align*}
            \dH(P_{X|W}(\cdot|w), P_X^{(k)}) & = \dH(P_{X|W}(\cdot|w), \sum_{w' \in \mathcal{W}_k} \lambda_{w'} P_{X|W}(\cdot | w'))           \\
                                             & \leq \sum_{w' \in \mathcal{W}_k} \lambda_{w'} \cdot \dH(P_{X|W}(\cdot|w),  P_{X|W}(\cdot | w')) \\
                                             & \leq Lh
        \end{align*}
    \end{block}
\end{frame}
